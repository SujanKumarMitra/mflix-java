...
public class QueryBuilders extends AbstractLesson {
    ...
    /**
     * This test is same as {@link QueryBuilders#testMultiplePredicates()},
     * but uses raw document as query.
     */
    @Test
    public void testMultiplePredicatesWithRawDocument() {
        /*
        ...
        Document query = new Document().append("$and", Arrays.asList(
                new Document("cast", "Tom Hanks"),
                new Document("year", new Document().append("$gte", 1990).append("$lt", 2005)),
                new Document("metacritic", new Document("$gte", 80))
        ));

        Spliterator<Document> iterator = moviesCollection.find(query)
                .projection(new Document().append("title", 1).append("_id", 0))
                .spliterator();
        
        Set<String> actualTitles = StreamSupport.stream(iterator, false)
                .map(d -> d.getString("title"))
                .collect(toCollection(HashSet::new));

        int expectedSize = 4;
        Assert.assertEquals(expectedSize, actualTitles.size());

        Set<String> expectedTitles = new HashSet<>(Arrays.asList(
                "Forrest Gump",
                "Toy Story",
                "Toy Story 2",
                "Saving Private Ryan"));
